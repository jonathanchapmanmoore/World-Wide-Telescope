...
namespace Microsoft.Research.Science.Data
{
    ...
    public abstract partial class DataSet : IDisposable, IEnumerable<Variable>
    {
        ...
        /// <summary>
        /// This methods is called when the variable is to be added to the <see cref="DataSet"/>.
        ...
        /// </summary>
        protected virtual void CheckOnAddVariable(Variable var)
        {
        }
        ...
        private ReadOnlyDimensionList CheckComplementarityConstraints(Changes proposedChanges)
        {
            ...
            * */
            ...
            if (!success)
            {
                ...
                Trace.WriteLineIf(DataSet.TraceDataSet.TraceError, "Dimensions " + sb.ToString() + "differ");
                if (n == 1)
        ...
        /// <remarks>
        /// <para>
        /// Opens the <see cref="DataSet"/> using given <paramref name="uri"/> and copies the current <see cref="DataSet"/> content
        ...
        /// The resulting <see cref="DataSet"/> is committed.
        /// </para>
        /// <para>
        /// The method is able to clone a <see cref="DataSet"/> only if it fits into a memory.
        ...
        {
            DataSet result = DataSetFactory.Create(targetUri);

            bool oldAutoCommit = result.IsAutocommitEnabled;
            result.IsAutocommitEnabled = false;
            DataSetSchema sch = this.GetSchema(SchemaVersion.Committed);

            Dictionary<int, int> IDs = new Dictionary<int, int>();

            foreach (VariableSchema v in sch.Variables)
            {
                if (v.ID == GlobalMetadataVariableID)
                {
                    // Copying global metadata
                    var dstGlobalMetadata = result.Metadata;
                ...
                Variable t = result.AddVariableByValue(this.variables.GetByID(v.ID), false);
                IDs.Add(v.ID, t.ID);
            }

            foreach (CoordinateSystemSchema cs in sch.CoordinateSystems)
            {
                var srcAxes = this.CoordinateSystems[cs.Name].AxesArray;
                Variable[] axes = new Variable[srcAxes.Length];
                for (int i = 0; i < axes.Length; i++)
                    axes[i] = result.Variables.GetByID(IDs[srcAxes[i].ID]);
                result.CreateCoordinateSystem(cs.Name, axes);
            }

            foreach (VariableSchema v in sch.Variables)
            {
                foreach (CoordinateSystem cs in this.variables.GetByID(v.ID).CoordinateSystems)
                    result.Variables.GetByID(IDs[v.ID]).AddCoordinateSystem(result.CoordinateSystems[cs.Name, SchemaVersion.Recent]);
            }

            result.Commit();
            result.IsAutocommitEnabled = oldAutoCommit;
