...
namespace Microsoft.Research.Science.Data.NetCDF4
{
    ...
    public class NetCDFDataSet : DataSet
    {
            ...
            : base(false, true)
        {
            ...
            DataSetUri.NormalizeFileNames(this.uri);
            ...
            try
            {
                ...
                if (exists)
                {
                    ...
                    if (openMode == ResourceOpenMode.ReadOnly)
                        res = NetCDF.nc_open(fileName, NetCDF.CreateMode.NC_NOWRITE /*| NetCDF.CreateMode.NC_SHARE*/, out ncid);
                    else
                        res = NetCDF.nc_open(fileName, NetCDF.CreateMode.NC_WRITE /*| NetCDF.CreateMode.NC_SHARE*/, out ncid);
                }
                else
                {
                    ...
                    res = NetCDF.nc_create(fileName, NetCDF.CreateMode.NC_NETCDF4 | NetCDF.CreateMode.NC_CLOBBER /*| NetCDF.CreateMode.NC_SHARE*/, out ncid);
                ...
                if (exists)
                {
                    ...
                    for (int i = 0; ; i++)
                    {
                        ...
                        NetCDF.NcType type;
                        IntPtr len;
                        res = NetCDF.nc_inq_att(ncid, NetCDF.NC_GLOBAL, attName, out type, out len);
                        if (res == (int)NetCDF.ResultCode.NC_ENOTATT) // that's all: no more cs
                            break;
                        if (res != (int)NetCDF.ResultCode.NC_NOERR) // an error has occurred
                            HandleResult(res);
                        ...
                        string attValue = NcGetAttText(ncid, NetCDF.NC_GLOBAL, attName, (int)len, out res);
                        HandleResult(res);
                        ...
                        var.Metadata.ForEach(
                            delegate(KeyValuePair<string, object> entry)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    throw new Exception("Coordinate system with name " + entry.Value + " not found", ex);
                                }
                            },
        ...
        protected override void OnTransactionOpened()
        {
            ...
            if (!initializing && rollbackEnabled)
            {
                ...
                res = NetCDF.nc_open(((NetCDFUri)this.uri).FileName, NetCDF.CreateMode.NC_WRITE, out ncid);
                HandleResult(res);
        ...
        protected override void OnPrecommit(DataSet.Changes changes)
        {
            ...
            if (proposedCS.Count > 0)
            {
                ...
                for (int i = 0; i < proposedCS.Count; i++)
                {
                    ...
                    WriteNetCdfAttribute(NetCDF.NC_GLOBAL, attName, attValue.ToString(), null);
                }
            }
        }
        /// <summary>
        ...
        private Variable ReadNetCdfVariable(int varid)
        {
            ...
            if (type == NetCDF.NcType.NC_INT64 || type == NetCDF.NcType.NC_DOUBLE)
            {
                ...
                int alen = (int)alen1;
                if (res == (int)NetCDF.ResultCode.NC_NOERR && atype == NetCDF.NcType.NC_CHAR)
                {
                ...
                else if (res != (int)NetCDF.ResultCode.NC_ENOTATT)
                {
                    HandleResult(res);
                }
            }
            else if (type == NetCDF.NcType.NC_UBYTE) // might be bool as well
            {
                NetCDF.NcType atype;
                IntPtr alen1;
            ...
            switch (type)
            {
                ...
                case NetCDF.NcType.NC_STRING:
                    ...
                    else if (customType && atm[aname] == typeof(string[]))
                    {
                        ...
                        {
                            IntPtr[] strdata = new IntPtr[length];
                            res = NetCDF.nc_get_att_string(NcId, varid, aname, strdata);
                            NetCDFDataSet.HandleResult(res);

                            string[] strings = new string[length];
                            for (int i = 0; i < length; i++)
                            {
                                strings[i] = UTF8Marshal.PtrToStringUTF8(strdata[i]);
        ...
        internal static string NcGetAttText(int ncid, int varid, string aname, int len, out int res)
        {
            ...
            IntPtr ptr = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(byte)) * len);
            res = NetCDF.nc_get_att_text(ncid, varid, aname, ptr);
            if (res == (int)NetCDF.ResultCode.NC_NOERR)
                strvalue = UTF8Marshal.PtrToStringUTF8(ptr, len);
        ...
        internal static int NcPutAttText(int ncid, int varid, string aname, string value)
        {
            if (value == null)
            {
                return NetCDF.nc_put_att_text(ncid, varid, aname, IntPtr.Zero, IntPtr.Zero);
            }
            ...
            IntPtr ptr = dataHandle.AddrOfPinnedObject(); //Marshal.AllocHGlobal(Marshal.SizeOf(typeof(byte)) * len);
            try
            {
                res = NetCDF.nc_put_att_text(ncid, varid, aname, new IntPtr(len), ptr);
            }
            finally
            {
                dataHandle.Free();
        ...
        internal void WriteNetCdfAttribute(int varid, string name, object value, AttributeTypeMap atm)
        {
            ...
            else if (value is string[])
            {
                ...
                res = NetCDF.nc_put_att_string(NcId, varid, name, new IntPtr(strArr.Length), strArr);
                HandleResult(res);
            }
            ...
            else if (value is bool)
            {
                ...
                res = NetCDF.nc_put_att_ubyte(NcId, varid, name, NetCDF.NcType.NC_UBYTE, new IntPtr(1), (new byte[] { bvalue }));
                HandleResult(res);
            }
            else if (value is bool[])
            {
                ...
                res = NetCDF.nc_put_att_ubyte(NcId, varid, name, NetCDF.NcType.NC_UBYTE, new IntPtr(bvalue.Length), bvalue);
                HandleResult(res);
            }
            else // Numeric types
            {
                ...
                if (value.GetType().GetElementType() == typeof(sbyte))
                    NetCDF.nc_put_att_schar(NcId, varid, name, NetCDF.NcType.NC_BYTE, new IntPtr(((sbyte[])value).Length), ((sbyte[])value));
                else if (value.GetType().GetElementType() == typeof(byte))
                    NetCDF.nc_put_att_ubyte(NcId, varid, name, NetCDF.NcType.NC_UBYTE, new IntPtr(((byte[])value).Length), ((byte[])value));
                else if (value.GetType().GetElementType() == typeof(short))
                    NetCDF.nc_put_att_short(NcId, varid, name, NetCDF.NcType.NC_SHORT, new IntPtr(((short[])value).Length), ((short[])value));
                else if (value.GetType().GetElementType() == typeof(int))
                    NetCDF.nc_put_att_int(NcId, varid, name, NetCDF.NcType.NC_INT, new IntPtr(((int[])value).Length), ((int[])value));
                else if (value.GetType().GetElementType() == typeof(ushort))
                    NetCDF.nc_put_att_ushort(NcId, varid, name, NetCDF.NcType.NC_USHORT, new IntPtr(((ushort[])value).Length), ((ushort[])value));
                else if (value.GetType().GetElementType() == typeof(uint))
                    NetCDF.nc_put_att_uint(NcId, varid, name, NetCDF.NcType.NC_UINT, new IntPtr(((uint[])value).Length), ((uint[])value));
                else if (value is float[])
                    NetCDF.nc_put_att_float(NcId, varid, name, NetCDF.NcType.NC_FLOAT, new IntPtr(((float[])value).Length), ((float[])value));
                else if (value is double[])
                    NetCDF.nc_put_att_double(NcId, varid, name, NetCDF.NcType.NC_DOUBLE, new IntPtr(((double[])value).Length), ((double[])value));
                else if (value.GetType().GetElementType() == typeof(Int64))
                    NetCDF.nc_put_att_longlong(NcId, varid, name, NetCDF.NcType.NC_INT64, new IntPtr(((Int64[])value).Length), ((Int64[])value));
                else if (value.GetType().GetElementType() == typeof(UInt64))
                    NetCDF.nc_put_att_ulonglong(NcId, varid, name, NetCDF.NcType.NC_UINT64, new IntPtr(((UInt64[])value).Length), ((UInt64[])value));
                else
        ...
        {
            if (resultCode == (int)NetCDF.ResultCode.NC_NOERR)
                return;
    ...
    internal class AttributeTypeMap
    {
        ...
        public AttributeTypeMap(int ncid, int varid)
        {
            ...
            if (resultCode == (int)NetCDF.ResultCode.NC_NOERR &&
                (type == NetCDF.NcType.NC_STRING || type == NetCDF.NcType.NC_CHAR))
            {
