...
namespace Microsoft.Research.Science.Data.NetCDF4
{
    ...
    public class NetCDFUri : DataSetUri
    {
        ...
        /// </summary>
        /// <remarks>
        /// Indicates whether the rollback is enabled or not. If it is, each transaction will lead to copying source file for further possible restoration. If it isn't, rollback will throw an exception in case of its invoke. Set it for data sets having not only NetCDF variables and consider set it false for very large files. Default is true.
        /// </remarks>
        [Description("Indicates whether the rollback is enabled or not (affects DataSet modification only).\nIf it is, each transaction will lead to copying source file for further possible restoration.\nIf it isn't, rollback will throw an exception in case of its invoke.\nSet it for data sets having not only NetCDF variables and\nconsider set it false for very large files.\nDefault is true.")]
        public bool EnableRollback
        {
            get
            {
                string val = GetParameterValue("enableRollback", "true").ToLower();
